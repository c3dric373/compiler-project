...
void CFG::cleanSymbolTable(AST::Bloc *bloc) {
    std::vector<std::tuple<AST::Bloc*, std::vector<std::string>>> var_names =
            bloc->get_var_names();
    for (auto &it : var_names) {
      std::vector<std::string> names = std::get<1>(it);
      AST::Bloc *bloc = std::get<0>(it);
      for(auto &name: names){
        this->deleteSymbol(bloc, name);
      }
